...
Thresh_Imextendedmin_Dist_Transf=0.87;  %5 for SIM4  0.75 hela2
...
figure,imagesc(label2rgb(Label_Map, 'prism','k', 'shuffle'));axis image; axis off, %title(titlestring,'color','k');
...
Thresh_solidity = 0.91;% 0.8926;   %SIM04  thres =0.8926 smallest other .95, 0.9 for Hela2
...
if ~isempty(multiCentroids)
    text(multiCentroids(:,1),multiCentroids(:,2), num2str(estimated_nb_Cells'), 'Color','b','FontWeight','bold', 'FontSize', 8)
end
hold off;
numberOfcells      = size(Cell_Features, 1);
figure,
%%%%%%%%%%PUT BACK %%%%%%%%%%%%%%set(gcf, 'Units','Normalized','OuterPosition',[0 0 .8 .8]);
colormap 'gray',
...
for k = 1 : numberOfBoundaries
    ...
    plot(thisBoundary(:,2), thisBoundary(:,1), 'g', 'LineWidth', 0.6);
end
hold off;
%%
textFontSize = 7;
...
for k = 1 : numberOfcells
    ...
    text(cellCentroid(1) + labelShiftX, cellCentroid(2), num2str(k),'color','y', 'FontSize',4,'Fontname', 'Cambria','FontWeight','bold','FontAngle','italic');
end
...
if individual_Cells
    ...
    for p = 1: numberOfcells           % Loop through all cells.
        ...
        % each cell its image
        subImage1 = imcrop(Original_Image, thiscellsBoundingBox);
        subImage2 = imcrop(Label_Map, thiscellsBoundingBox);
        % Determine if it's a new (small) or a smalll (large cell) include
        % the cell clusters from solidity
        if Cell_Features(p).Area > 1500
         cellType = 'large';
        %else
        %cellType = 'new';
        %end
        ...
        imagesc(subImage1);colormap gray, axis image; axis off,
        caption = sprintf('cell #%d \nPerimeter = %.1f pixels\nArea = %d pixels', ...
            p,  Cell_Features(p).Perimeter, Cell_Features(p).Area);
        title(caption);
% % % %         figure, %colormap 'gray',   %subplot(3, 4, p);
% % % %         imagesc(subImage2);colormap gray, axis image; axis off,
% % % %         caption = sprintf('cell #%d \nPerimeter = %.1f pixels\nArea = %d pixels', ...
% % % %             p,  Cell_Features(p).Perimeter, Cell_Features(p).Area);
% % % %         title(caption);
        end;
...
%title(titlestring,'color','k','FontSize', 12, 'Fontname', 'Cambria');
...
titlestring='Cell separation oversegmentation';
...
figure, imshowpair(Intensity_Image, Label_Dist_Transf2 == 0,'falsecolor');titlestring='intensity im and watershed of minima imposed';
%title(titlestring,'color','k');
...
if numel(unique(New_Label_Map))==2
        CC_Structure     = bwconncomp(New_Label_Map, 4);
        New_Label_Map    = labelmatrix(CC_Structure);  % is a mapping Create different label to each cell
        New_Label_Map    = double(New_Label_Map);
    else
        CC_Structure     = bwconncomp(New_Label_Map>0, 4);
    end
